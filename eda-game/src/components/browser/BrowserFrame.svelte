<script lang="ts">
  import type { SearchResult, ToolId, ToolQueryMap, SearchEngineData, StateMachineData } from '$lib/engine/types-v2';
  ...
  function runGsxt() {
    ...
    if (match) {
      if (typeof match.results === 'string') {
        ...
        toolResult = match.results.map((r) => {
  ...
  <div class="browser-content">
        ...
        {#if searchResults.length > 0}
          <div class="search-results">
            {#each searchResults as result}
              <div class="result-item" class:useful={result.useful} class:clickable={result.clickable} onclick={() => openSearchResult(result)} role="button" tabindex="0" onkeydown={(e) => e.key === 'Enter' && openSearchResult(result)}>
                <div class="result-title">{result.title}</div>
                <div class="result-source">{result.source}</div>
                <div class="result-summary">{result.summary}</div>
                {#if result.key_clue}<div class="result-clue">🔑 {result.key_clue}</div>{/if}
              </div>
            {/each}
          </div>
        {:else if query}
    ...
    {:else if view === 'whois'}
      <div class="tool-page">
        ...
        {#if toolResult}<pre class="tool-result">{toolResult}</pre>{/if}
      </div>
    {:else if view === 'gsxt'}
      <div class="tool-page">
        <h3>国家企业信用信息公示系统</h3>
    ...
    {:else if view === 'page'}
      <div class="tool-page">
        ...
        <pre class="tool-result">{toolResult}</pre>
      </div>
    {/if}
  </div>
</div>
